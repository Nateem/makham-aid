...
$arr["ERROR"]=true;
if($TYPES=="SELECT_reportSome_customer"){
	...
	if($DateStart && $DateEnd && $CODE){
			...
			$Query = mysqli_query($DB,
				"SELECT DISTINCT customer_value.company_ID,customer_value.title_ID,customer_value.store_type_ID,SUM(customer_value.VALUE) AS SUM_VALUE,company.NAME_FULL,company.NAME_SHORT
				FROM customer_value
				LEFT JOIN company
			...
			$Query5 = mysqli_query($DB,
				...
				AND customer_value.MASCODE='{$CODE}'
				$AND
				GROUP BY customer_value.company_ID,customer_value.store_type_ID,customer_value.title_ID
				");
			$DATA5 = [];
			...
			$Query1 = mysqli_query($DB,
				"SELECT DISTINCT customer_value.store_type_ID,customer_value.title_ID,customer_value.company_ID,store_type.NAME,SUM(customer_value.VALUE) AS SUM_VALUE
				FROM customer_value
