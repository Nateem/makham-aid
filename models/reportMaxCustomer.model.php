...
$arr["ERROR"]=true;
if($TYPES=="SELECT_reportMaxCustomer"){
	...
	if($DateStart && $DateEnd){
			...
			while($Fetch=mysqli_fetch_assoc($Query)){
				...
				$Query2 = mysqli_query($DB,
					"SELECT SUM(customer_value.VALUE) AS MAX_SUM_VALUE,CONCAT(COALESCE(customer.MASCODE,''),' : ',COALESCE(customer.STNA,''),COALESCE(customer.NAME,''),' ',COALESCE(customer.SURNAME,'')) AS customer_NAME
					FROM customer_value
					LEFT JOIN customer
					...
					$AND
					GROUP BY customer_value.MASCODE,customer_value.store_type_ID
